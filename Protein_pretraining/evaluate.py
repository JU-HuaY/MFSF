...
def get_results(binding_sites, pred_binding_sites):
    from sklearn.metrics import confusion_matrix
    
    T_TP, T_TN, T_FP, T_FN = 0, 0, 0, 0
    
    for bs, bps in zip(binding_sites, pred_binding_sites):
        conf_matrix = confusion_matrix(bs, bps)
        ...
        T_FP += FP
        T_FN += FN
    T_TP, T_TN, T_FP, T_FN = int(T_TP), int(T_TN), int(T_FP), int(T_FN)
    ...
    if (T_TP + T_FP) * (T_TP + T_FN) * (T_TN + T_FP) * (T_TN + T_FN) == 0:
        MCC = 0
    else:
        MCC = float(T_TP * T_TN - T_FP * T_FN) / (np.sqrt(float((T_TP + T_FP) * (T_TP + T_FN) * (T_TN + T_FP) * (T_TN + T_FN))))
    return np.round(specificity, 2), np.round(ACC, 2), MCC, np.round(F1_score, 2)
