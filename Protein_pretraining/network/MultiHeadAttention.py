...
def mask_topk_attention_weights(attention_weights, K):

    # get top K valuse and indices
    topk_values, topk_indices = torch.topk(attention_weights, K, dim=-1)

    # create new mask and use scatter_
    mask = torch.zeros_like(attention_weights, dtype=torch.bool, device=attention_weights.device)
...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, dropout=None):
        ...
        if self.K is not None:
            p_attn = mask_topk_attention_weights(p_attn, self.K)
        
        if dropout is not None:
            p_attns = dropout(p_attn)

        return torch.matmul(p_attns, value), mask_topk_attention_weights(scores, self.K)
...
class Covariance_Attention(nn.Module):
    ...
    def forward(self, qk, v, mask=None, dropout=None):
    	...
    	if dropout is not None:
    	    p_attns = dropout(p_attn)
    	return torch.matmul(p_attns, v), scores
...
class Attention_ADJ(nn.Module):
    ...
    def forward(self, qk, v, mask=None):
        ...
        # 1) Do all the linear projections in batch from d_model => h x d_k
        querys = self.q_layers(qk)
        keys = self.k_layers(qk)
        values = self.v_layers(v)
        # 2) Apply attention on all the projected vectors in batch.
        x, attn = self.attention(querys, keys, values, mask=mask, dropout=self.dropout)
        return x, attn
